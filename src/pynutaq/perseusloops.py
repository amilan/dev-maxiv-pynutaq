...
class PerseusLoops(object):
    ...
    def __init__(self):
            ...
            self.mo1000 = Mo1000(self._board_state)

            print "Mi125 2 initialization..."
            self.mi125 = Mi125(self._board_state)
            print "DONE"
    ...
    def connect(self):
        try:
            ret = eapi.connect_cce(PERSEUS_LOOP_IP, self._board_state)
            if ret < 0:
    ...
    def custom_write(self, register, data):
        ...
        """
        try:
            ret = eapi.custom_register_write_send(self._board_state, register, data)
            if ret < 0:
                raise adp_exception(ret)
        except Exception, e:
            raise e
    ...
    def write(self, address, value):
        try:
            ret = eapi.memory_write_send(self._board_state, address, value)
            if ret < 0:
                raise adp_exception(ret)
        except Exception, e:
            raise e
    ...
    def read(self, address):
        try:
            ret, value = eapi.memory_read_send(self._board_state, address)
            if ret < 0:
                raise adp_exception(ret)
            return value
        except Exception, e:
